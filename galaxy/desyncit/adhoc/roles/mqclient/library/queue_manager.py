...
def main():
    ...
    if module.params['qmname'][0] == "ALL_QMGRS":
        dspmq_path = module.get_bin_path('dspmq', required=True)
        rc, stdout, stderr = module.run_command([dspmq_path])
        module.params['qmname'] = re.findall(r"(?<=QMNAME\()([^\)]*)", stdout)
        result['qmlists'] = module.params['qmname']
