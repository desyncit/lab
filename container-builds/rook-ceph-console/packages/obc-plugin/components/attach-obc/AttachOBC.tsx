...
import { CephObjectBucketClaimModel } from '@odf/shared';
    ...
    try {
      ...
      if (createOBC) {
        const obj = await k8sCreate({
          model: CephObjectBucketClaimModel,
          data: state.payload,
        });
        obc = getName(obj);
      }
          ...
          <FormGroupController
            ...
            render={({ onChange }) => (
              <ResourceDropdown
                ...
                resourceModel={CephObjectBucketClaimModel}
                resource={{
                  kind: referenceForModel(CephObjectBucketClaimModel),
                  namespace,
                  namespaced: true,
                  isList: true,
                }}
