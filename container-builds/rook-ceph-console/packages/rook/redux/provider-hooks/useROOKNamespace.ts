...
import { useROOKNamespaceDispatch } from '../dispatchers';

const CLIENT_SUBSCRIPTION_NAME = 'ocs-client-operator';

const getSpecName = (resource: SubscriptionKind) => resource?.spec?.name;

const ROOK_SUBSCRIPTION_NAME = 'odf-operator';

export const FDF_FLAG = 'FDF_FLAG'; // Based on whether installed operator is ROOK or FDF
export const SCALE_GATE_FLAG = 'SCALE_GATE'; // Set to "true" if ROOK operator is installed
  ...
  while (shouldRetry) {
    ...
    try {
      ...
      const odfSubscription = subscriptions.find(
        (subscription) => getSpecName(subscription) === ROOK_SUBSCRIPTION_NAME
      );
      const isROOKPresent = odfSubscription !== undefined;
      if (isROOKPresent) {
      ...
      if (!ns) throw new Error('ROOK install namespace not found');
    } catch (err) {
...
export const useROOKNamespace = (setFlag: SetFeatureFlag): void => {
  const dispatch = useROOKNamespaceDispatch();
  ...
  React.useEffect(() => {
    namespaceDetector()
      ...
      /**
       * ROOK can be installed in any namespace, still recommended namespace is "openshift-storage".
