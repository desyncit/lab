...
import { RedExclamationCircleIcon } from '@rook/shared/status/icons';
import StatusIconAndText from '@rook/shared/status/StatusIconAndText';
import { useCustomTranslation } from '@rook/shared/useCustomTranslationHook';
import { getAlertSeverity } from '@rook/shared/utils';
import {
  Alert,
  AlertSeverity,
...
const AlertBadge: React.FC<AlertBadgeProps> = ({
  ...
  return (
    <Button
      ...
      isInline
      onClick={onClick}
      className="rook-alerts-panel__button"
    >
    <Badge
      key={key}
      className={`rook-alerts-panel__badge odf-alerts-panel__badge-${alertSeverity}`}
    >
      <StatusIconAndText title={alerts.length.toString()} icon={icon} />
    </Badge>
    <span className="rook-alerts-panel__badge-text">
...
const AlertsPanel: React.FC<AlertsProps> = ({
  ...
  return (
    <div
      className={classNames('rook-m-pane__body', 'rook-alerts__panel', className)}
    >
      <Title headingLevel="h3">
        {t('Alerts')} ({filteredAlerts.length})
        {!!titleToolTip && <FieldLevelHelp>{titleToolTip}</FieldLevelHelp>}
      </Title>
      <Divider className="rook-alerts-panel__divider" />
      <AlertBadge
